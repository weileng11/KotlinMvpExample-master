...
import com.exmple.baseprojectmvp.mvp.adapter.VideoDetailAdapter
...
import com.exmple.baseprojectmvp.mvp.presenter.VideoDetailPresenter
import com.exmple.baseprojectmvp.widget.Constants
import com.exmple.baseprojectmvp.widget.VideoListener
...
import com.exmple.corelib.utils.CleanLeakUtils
 ...
 * @time:  15:50
 */
 class VideoDetailActivity : BaseMvpActivity<IVideoDetailContract.View, IVideoDetailContract.Presenter>(), IVideoDetailContract.View {

   override var mPresenter: IVideoDetailContract.Presenter = VideoDetailPresenter()
...
//   private val mAdapter by lazy { VideoDetailAdapter(this, itemList) }
   ...
   //设置第一次请求的数据
   private var demoAdapter : VideoDetailAdapter? = null
   ...
   private var mMaterialHeader: MaterialHeader? = null
    ...
    override fun initView() {
       ...
       mRecyclerView.layoutManager = LinearLayoutManager(this)
        demoAdapter= VideoDetailAdapter(dataList = itemList)
       mRecyclerView.adapter = demoAdapter

       //设置相关视频 Item 的点击事件
       demoAdapter?.setOnItemDetailClick { mPresenter.loadVideoInfo(it) }
...
//              .centerCrop()
   ...
   private fun saveWatchVideoHistoryInfo(watchItem: HomeBean.Issue.Item) {
      ...
      val historyMap = WatchHistoryUtils.getAll(Constants.FILE_WATCH_HISTORY_NAME,LibApplication.mContext) as Map<*, *>
      for ((key, _) in historyMap) {
         if (watchItem == WatchHistoryUtils.getObject(Constants.FILE_WATCH_HISTORY_NAME,LibApplication.mContext, key as String)) {
            WatchHistoryUtils.remove(Constants.FILE_WATCH_HISTORY_NAME,LibApplication.mContext, key)
         }
      }
      WatchHistoryUtils.putObject(Constants.FILE_WATCH_HISTORY_NAME,LibApplication.mContext, watchItem,"" + mFormat.format(Date()))
   }
    ...
    * 设置相关的数据视频
    */
   override fun setRecentRelatedVideo(List: ArrayList<HomeBean.Issue.Item>) {
...
//              .format(DecodeFormat.PREFER_ARGB_8888)
    ...
    */
   fun loadVideoInfo() {
      mPresenter.loadVideoInfo(this!!.itemData!!)
   }
